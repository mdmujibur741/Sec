...
watch(search, value => {
       router.get('/admin/submissions', {search: value, perPage: perPage.value},{
          preserveState : true,
          replace :true,
          preserveScroll: true
       });
...
function getPage(){
  router.get('/admin/submissions', {search: search.value, perPage: perPage.value},{
          preserveState : true,
          replace :true,
          preserveScroll: true
       });
...
function pdf($id){
     router.get(route('web.pdf',$id),{
        onSuccess: ()=> router('admin.submissions.index')
     })
}
            ...
            <div class="flex justify-between my-1  gap-5  p-3">
              ...
              <table class="w-full" >
                ...
                <tbody class="bg-white" >
                  <tr v-for="(sub, index) in props.submissions.data"  class="text-gray-700">
                    ...
                    <td class="px-4 py-2 border ">
                     ...
                     <div>  <Link :href="route('admin.submissions.show',sub.id)" class="btn-show"> <i class="fa-solid fa-eye"></i></Link></div>
                     <div>  <Link :href="route('admin.submissions.edit',sub.id)" class="btn-edit"> <i class="fa-solid fa-pen-to-square"></i></Link></div>
                     ...
                     <div>  <a :href="route('web.download.pdf',sub.id)" class="btn-download"> <i class="fa-solid fa-file-arrow-down"></i> </a></div>
                      <Link :href="route('admin.submissions.destroy',sub.id)" onclick="return confirm('Are you sure you want to delete this item?');" as="button" type="button" method="delete" class="btn-delete"> <i class="fa-solid fa-trash"></i></Link>
                      </div>
                  </td>
                  </tr>
             ...
             <Pagination :links="props.submissions.meta.links"/>
